<h2>Caution:</h2>
<h4> 因為本系統使用WebRTC的功能，請使用Google Chrome開啟</h4>
<h2>Introduce:</h2>
<pre>
此系統主要是一個線上的語音聊天程式，透過P2P的方式，讓兩端的使用者能夠在質量與安全的保證之下有驚艷的聊天體驗。
而系統主要有兩個部分，一個是線上配對系統，藉由使用者提供的基本資料來進行配對。另一個是在配對完成之後，
會讓兩端使用者進入一個「你畫我猜」的小遊戲，遊玩此遊戲的目的是在於讓兩端使用不要一開始就進行語音聊天產生尷尬情況，
透過一個60秒的小遊戲之後，雙方再決定要不要進行通話，讓對方可以至少有一個能夠開啟的話題。
而不論雙方是否願意進行通話（在第一輪遊戲結束之後），都可以繼續玩你畫我猜的遊戲，而系統會在每一輪遊戲結束後詢問是否要進行通話。
然而當其中一方如果覺得不想再進行下去想離開換一個人或是離開瀏覽器，被斷線那方也會再回到準備配對的畫面以進行新的配對。
</pre>
